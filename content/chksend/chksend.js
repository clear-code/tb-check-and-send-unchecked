...
function CheckAndSend()
{
	...
	this.mObserver = new MutationObserver(function(mutations) {
  	mutations.forEach(function(mutation) {
    	if (mutation.removedNodes && mutation.removedNodes.length > 0) me.clearAllHighlightsWrapper();
  	});   
	});
	
	var mConfig = { attributes: true, childList: true, characterData: true };
	this.mObserver.observe(this.note, mConfig);
...
CheckAndSend.prototype.showAlertForAttachmentsSize = function(size)
{
	...
	var buttonPressed = this.promptService.confirmEx(window, 
              title, 
              message,
              (this.promptService.BUTTON_TITLE_OK * this.promptService.BUTTON_POS_0) +
...
CASRecipientsChecker.prototype.checkAddress = function(bySender, byAddrBook)
{
	...
	if (!noCheck && byAddrBook && uri != "") {
		if (uri == "all") {
			var subDirs = this.addrBook.directories;
	...
	for (var key in this.checkList) {
		...
		if (noCheck && this.checkList[key] == 0) highlightColor ="red";
		else if (bySender && this.checkList[key] == 2) highlightColor = "blue";
